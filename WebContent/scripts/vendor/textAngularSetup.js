...
textAngularSetup.value('taOptions',  {
	toolbar: [
		...
		['copy','paste'],
		['indent','outdent'],
		['azulFont','rojoFont','grisFont','blancoFont','negroFont','highlightFont','azulBack','rojoBack','grisBack','blancoBack','negroBack', 'highlightBack']
	],
	classes: {
		focussed: "focussed",
		toolbar: "btn-toolbar",
...
textAngularSetup.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', function(taRegisterTool, $window, taTranslations, taSelection){
	...
	var colores = [];
	colores["rojo"] = "rgb(255, 0, 0)";
	colores["azul"] = "rgb(0, 0, 255)";
	colores["gris"] = "rgb(128, 128, 128)";
	colores["blanco"] = "rgb(255, 255, 255)";
	colores["negro"] = "rgb(85, 85, 85)";
	colores["highlight"] = "rgb(255, 255, 0)";
	
	//Colorear la letra y fondo
	for ( var nombreColor in colores) {
		
		taRegisterTool(nombreColor+"Font", {
			color: colores[nombreColor],
			iconclass: 'font icon '+nombreColor,
			action: function(){
				return this.$editor().wrapSelection("forecolor", this.color) ;
			},
			activeState: function(){
				return document.queryCommandValue('forecolor')== this.color;
			}
		});
		
		taRegisterTool(nombreColor+"Back", {
			color: colores[nombreColor],
			iconclass: "font icon "+nombreColor+"_back",
			action: function(){
				return this.$editor().wrapSelection("backcolor", this.color) ;
			},
			activeState: function(){
				return document.queryCommandValue('backcolor')== this.color;
			}
		});
